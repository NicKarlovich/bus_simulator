...
bool rLocalSimulator::Start() {
  ...
  // CC Eastbound stops
  Stop * stop_CC_EB_1 = new Stop(0, 44.972392, -93.243774);  // Westb station
  Stop * stop_CC_EB_2 = new Stop(1, 44.973580, -93.235071);  // su station
  Stop * stop_CC_EB_3 = new Stop(2, 44.975392, -93.226632);  // Oak & Wash
  ...
  // TODO(staff): is this always true? If so, we may want to reduce the length
  ...
  RtestPassengerGenerator * CC_EB_generator = new RtestPassengerGenerator
  (CC_EB_probs, CC_EB_stops_list);
  RtestPassengerGenerator * CC_WB_generator = new RtestPassengerGenerator
  (CC_WB_probs, CC_WB_stops_list);
