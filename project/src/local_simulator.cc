...
LocalSimulator::LocalSimulator(std::ostream &out) : out_(out) { }
/*
LocalSimulator::LocalSimulator(std::ostream &out) {
  out_ = out;
}*/
...
bool LocalSimulator::Start() {
  ...
  prototype_routes_[0]->Report(out_);
  prototype_routes_[1]->Report(out_);
...
bool LocalSimulator::Update() {
  ...
  out_ << "~~~~~~~~~~~~~ The time is now " << simulation_time_elapsed_;
  out_ << " ~~~~~~~~~~~~~" <<  std::endl;
