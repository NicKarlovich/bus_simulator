    ...
    for (std::vector<Passenger *>::iterator it = passengers.begin();
     it != passengers.end(); it++) {
      if ((*it)->GetDestination() != time) {
        ...
        // When removing the passenger, the iterator
        // gets incremented to the next element.
        // We need to push it back so that the for
        // loop increment places the iterator at
