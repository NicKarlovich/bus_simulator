...
ArrayPointerTest::ArrayPointerTest(int count) {
  ...
  cout << sizeof(ducks1_[count_]) << endl;
  //ducks1_ = new Duck [count_];
  //ducks1 doesn't need to be created because?...
  ...
  for(int i = 0; i < count_; i++) {
    ducks4_[i] = new Duck();
  }//makes a pointer to an array of pointers which point to ducks.
